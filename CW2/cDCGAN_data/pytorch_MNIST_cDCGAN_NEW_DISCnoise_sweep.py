...
for std_ in [0.005, 0.01, 0.05, 0.1, 0.2]:
    # network
    G = generator(64)
    D = discriminator(64)
    ...
    for epoch in range(train_epoch):
        ...
        for x_, y_ in train_loader:
            ...
            D_result = D(x_+std_*torch.randn(x_.size()).cuda(), y_fill_).squeeze()
            ...
            D_result = D(G_result+std_*torch.randn(x_.size()).cuda(), y_fill_).squeeze()
        ...
        fixed_p = root + 'Fixed_results/' + model + 'std_' + str(std_) + '_' + str(epoch + 1) + '.png'
    ...
    np.savetxt('MNIST_cDCGAN_results/Losses/GLoss_std_' + str(std_) + '.txt',epoch_G_losses,delimiter='\n') # SAVE THESE RESULTS WHERE?
    np.savetxt('MNIST_cDCGAN_results/Losses/DLoss_std_' + str(std_) + '.txt',epoch_D_losses,delimiter='\n')
