...
for epoch in range(train_epoch):
    ...
    #size_figure_grid = num_test
    #fig, ax = plt.subplots(10, size_figure_grid, figsize=(5, 5))
    #for i, j in itertools.product(range(10), range(size_figure_grid)):
    #    ax[i, j].get_xaxis().set_visible(False)
    #    ax[i, j].get_yaxis().set_visible(False)
    
    #for k in range(10*num_test):
    #    i = k // num_test
    #    j = k % num_test
    #    ax[i, j].cla()
    #    ax[i, j].imshow(test_images[k, 0].cpu().data.numpy(), cmap='gray')
