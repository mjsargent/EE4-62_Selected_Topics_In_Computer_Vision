...
#G_lrates = [0.002, 0.0002, 0.00002]
#D_lrates = [0.002, 0.0002, 0.00002]
stdStore = [0.005, 0.01, 0.05, 0.1, 0.5]
for std in stdStore:
    ...
    for epoch in range(train_epoch):
        ...
        for x_, y_ in train_loader:
            ...
            D_result = D(x_+std*torch.randn(x_.size()).cuda(), y_fill_).squeeze()
            ...
            D_result = D(G_result+std*torch.randn(x_.size()).cuda(), y_fill_).squeeze()
    ...
    os.makedirs('MNIST_cDCGAN_results/std{}_results'.format(std), exist_ok=True)
    np.savetxt('MNIST_cDCGAN_results/std{}_results/GLoss.txt'.format(std),epoch_G_losses,delimiter='\n') # SAVE THESE RESULTS WHERE?
    np.savetxt('MNIST_cDCGAN_results/std{}_results/DLoss.txt'.format(std),epoch_D_losses,delimiter='\n')
    ...
    np.savetxt('MNIST_cDCGAN_results/std{}_results/performance.txt'.format(std),[total_store])
    np.savetxt('MNIST_cDCGAN_results/std{}_results/mean_inc.txt'.format(std),mean_inception_store)
    np.savetxt('MNIST_cDCGAN_results/std{}_results/std_inc.txt'.format(std),std_inception_store)
