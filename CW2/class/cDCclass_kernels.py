...
nkStore = [8,16,32,64,128]
for nk in nkStore:

    # network
    G = generator(nk) # CHANGED
    D = discriminator(nk)
    ...
    os.makedirs('MNIST_cDCGAN_results/nk{}_results'.format(nk), exist_ok=True)
    np.savetxt('MNIST_cDCGAN_results/nk{}_results/GLoss.txt'.format(nk),epoch_G_losses,delimiter='\n') # SAVE THESE RESULTS WHERE?
    np.savetxt('MNIST_cDCGAN_results/nk{}_results/DLoss.txt'.format(nk),epoch_D_losses,delimiter='\n')
    ...
    np.savetxt('MNIST_cDCGAN_results/nk{}_results/performance.txt'.format(nk),[total_store])
    np.savetxt('MNIST_cDCGAN_results/nk{}_results/mean_inc.txt'.format(nk),mean_inception_store)
    np.savetxt('MNIST_cDCGAN_results/nk{}_results/std_inc.txt'.format(nk),std_inception_store)
