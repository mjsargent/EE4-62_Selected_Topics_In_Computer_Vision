...
fixed_z_ = temp_z_
fixed_y_ = torch.zeros(num_test, 1).long()
for i in range(9):
    temp_z_ = torch.randn(num_test, 100)
    fixed_z_ = torch.cat([fixed_z_, temp_z_], 0)
    temp = torch.ones(num_test, 1).long() + i
    fixed_y_ = torch.cat([fixed_y_, temp], 0)
...
#    if(n == 0):    
#        test_images = G(fixed_z_[0:num_test//100], fixed_y_label_[0:num_test//100]).detach()
#    else:
#        test_images = torch.cat([test_images, G(fixed_z_[n*(num_test//100):(n+1)*(num_test//100)], fixed_y_label_[n*(num_test//100):(n+1)*(num_test//100)]).detach()])
    


batch_perm = np.random.permutation(num_test*10)

# Train and test LeNet for differing ratios ----
# 1 change proportion of synthetic
for real_proportion in [1,5,10,15,20]:#[0.1,0.2,0.5,1.0]:
    newLeNet = LeNet5().cuda()
    newLeNet.train()
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(newLeNet.parameters(), lr=2e-3)
 # create synthetic batch
    for syn_epoch in range(16):
        for n in range(468):
            ...
            G_result = G(z_, y_label_)
            optimizer.zero_grad()
            output_synthetic = newLeNet(G_result)
            loss = criterion(output_synthetic, y_)
            loss.backward()
            optimizer.step()
            
                
    optimizer = optim.Adam(newLeNet.parameters(), lr = 2e-4)
   
    # train network
    for epoch in range(real_proportion):
        track = 0
        for i, (images, labels) in enumerate(train_loader):
            ...
            # forward prop real then synthetic
            output_real = newLeNet(images)
            loss = criterion(output_real, labels)
            loss.backward()
            optimizer.step()
            #loss += criterion(output_synthetic, synthetic_labels.view(-1))
    ...
    for i, (images, labels) in enumerate(test_loader):
        labels.cuda()
        output = newLeNet(images.cuda())
        pred = output.detach().max(1)[1]
        total_correct += pred.eq(labels.cuda()).sum()
        total_images += len(labels)
    acc = total_correct.item()/total_images

    #print('Test Avg. Loss: %f, Accuracy: %f' % (avg_loss.detach().cpu().item(), float(total_correct) / total_images))
    print('Testing: Accuracy = {} '.format(float(acc)))
    os.makedirs('Q4altflip/rProp{}'.format(real_proportion), exist_ok=True)
    np.savetxt('Q4altflip/rProp{}/Acc.txt'.format(real_proportion), [acc],delimiter='\n')
