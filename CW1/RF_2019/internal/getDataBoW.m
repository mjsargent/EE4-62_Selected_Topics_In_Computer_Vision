...
[~, cent] = kmeans(desc_sel', numBins);
% write your own codes here
% ...

disp('Encoding Images...')

BoW = zeros(numBins,150);
imTrack = 1;
% Vector Quantisation
for i = 1:10
    for j = 1:15
        im = desc_tr(i,j);
        im = im{1};
        BoWIDX = knnsearch(cent,im');
        for k = 1:length(BoWIDX)
            %BoW(BoWIDX(k),(i-1)*j + j) = BoW(BoWIDX(k),(i-1)*j + j) + 1;
            BoW(BoWIDX(k),imTrack) = BoW(BoWIDX(k),imTrack) + 1;
